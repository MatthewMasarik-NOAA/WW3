...
9000 FORMAT (' TEST W3NDAT : SETTING UP FOR ',I4,' GRIDS')
  ...
  SUBROUTINE W3DIMW  ( IMOD, NDSE, NDST, F_ONLY )
    ...
    ALLOCATE ( WDATAS(IMOD)%WLV(NSEA),                              &
         ...
         WDATAS(IMOD)%UST(0:NSEA),                          &
         WDATAS(IMOD)%USTDIR(0:NSEA),                       &
         WDATAS(IMOD)%ASF(NSEA),                            &
         WDATAS(IMOD)%FPIS(NSEA), STAT=ISTAT                )
    CHECK_ALLOC_STATUS ( ISTAT )
    ...
    WDATAS(IMOD)%UST   (0:NSEA) = 1.E-5
    WDATAS(IMOD)%USTDIR(0:NSEA) = 0.
    WDATAS(IMOD)%ASF   (:) = 0.
    WDATAS(IMOD)%FPIS  (:) = 0.
